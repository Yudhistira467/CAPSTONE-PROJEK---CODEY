...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "codey-capstone-90a96eecae2d.json"
...
def get_recommendation():
    try:
        ...
        # Memeriksa dan hanya mengambil instruksi yang tidak kosong
        instructions = [entry['instruction'] for entry in user_data['answers'] if entry.get('instruction')]
        
        if not instructions:
            logging.warning(f"No valid instructions found for userId: {user_id}")
            return jsonify({"error": "No valid instructions found for the given userId"}), 400
        
        # Gabungkan semua instruksi menjadi satu teks
        combined_instructions = " ".join(instructions)
        logging.info(f"Combined instructions: {combined_instructions}")
        ...
        # Pra-pemrosesan combined_instructions
        input_data = preprocess_text(combined_instructions, tokenizer)
